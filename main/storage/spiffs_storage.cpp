...
esp_err_t SPIFFS::register_spiffs()
{
    ...
    if (!esp_spiffs_mounted(m_spiffs_conf.partition_label))
    {
        ...
        ret = esp_spiffs_info(conf.partition_label, &file_system_size, &used_bytes);
        ...
        {
            Output::log(e_log_type::et_WARN, "Number of used bytes cannot be larger than total");
...
esp_err_t SPIFFS::check_consistacny_spiffs()
{
    ...
    Output::log(e_log_type::et_WARN, "Performing SPIFFS_check() This can take some time ...");
    ...
    if (!Output::esp_result_handler(e_abort_handle::et_NOT_THROW, ret, m_TAG, "esp_spiffs_format"))
    {
        ...
        Output::log(e_log_type::et_WARN, "Performing SPIFFS_check() Failed, we are formating new partion, see M_D_FORMAT_NEW_PARTION_AFTER_CONSTANCY_FAILED...");
        ret = esp_spiffs_format(m_spiffs_conf.partition_label);
        Output::esp_result_handler(e_abort_handle::et_THROW, ret, m_TAG, "esp_spiffs_format");
